    ...
    <div className='flex flex-col lg:flex-row gap-6 h-ful'>
      ...
      <div className="w-full lg:w-115 shrink-0">
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <form action="" className="sapce-y-5" onSubmit={handleSchedule}>
            <div className="mb-5">
              <label htmlFor="" className="block text-xs text-slate-500 uppercase mb-2">Plaforms</label>
            ...
            <div className="">
              ...
              <textarea required rows={5} placeholder='What do you want to share today' className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 text-sm plceholder-slate-400 outline-none resize-none" value={content} onChange={(e)=> setContent(e.target.value)}/>
              ...
              {mediaFile ? (
                <div className="relative border border-slate-200 bg-slate-50 rounded-xl overflow-hidden">
                  {mediaFile.type.startsWith("image/") ? <img src={URL.createObjectURL(mediaFile)} alt="preview" className="w-full h-40 object-cover" />:
                   <video src={URL.createObjectURL(mediaFile)} className="w-full h-40 object-cover" controls></video> }
                   <button type='button' onClick={()=> setMediaFile(null)} className="absolute top-2 right-2 sizw-7 bg-slate-900/60 hover:bg-slate-900/80 text-white rounded-full flex items-center justify-center transition-colors">
            ...
            <div className="grid gird-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label htmlFor="" className="bolck text-xs text-slate-500 uppercase mb-2">Date</label>
                      <div className="relative">
                        <CalendarIcon className='size-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none' />
                        <input type="date" required className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border-slate-200 rounded-lg text-slate-900 text-sm outline-none" onChange={(e)=> setScheduledDate(e.target.value)} value={scheduledDate} />
                      </div>
                  </div>
                  <div>
                    <label htmlFor="" className="bolck text-xs text-slate-500 uppercase mb-2">Time</label>
                      <div className="relative">
                        <ClockIcon className='size-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none' />
                        <input type="time" required className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border-slate-200 rounded-lg text-slate-900 text-sm outline-none" onChange={(e)=> setScheduledTime(e.target.value)} value={scheduledTime} />
                      </div>
                  </div>
            </div>
            <button type='submit' disabled={loading} className="w-full flex items-center mt-3 justify-center gap-2 py-3.5 bg-red-500 hover:bg-red-600 transition-all text-white rounded-lg">
              {loading ? (
                <>
      ...
      <div className="flex-1 flex-col-gap-6 min-w-0">
        <div className="bg-white border  border-slate-200 rounded-2xl overflow-hidden">
         ...
         <div className="bg-white mt-5 border border-slate-200 rounded-2xl overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100">
            <SendIcon className='size-4 text-zinc-500' />
            <h3 className="text-sm text-slate-900">Published</h3>
            <span className="ml-auto text-xs font-bold bg-zinc-100 text-zinc-700 px-2 py-0.5 rounded-full">
              {published.length}
            </span>
          </div>
