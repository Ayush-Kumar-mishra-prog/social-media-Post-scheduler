// import { AlertCircleIcon, CheckCheckIcon, CheckCircleIcon, PlusIcon, UnplugIcon } from "lucide-react";
...
//     const handleDisconnect = async(accounId:string)=> {
//         const confirm = window.confirm("Are you sure to disconnect this accounts");
//         if(!confirm) return;
//         await onDisconnect(accounId)
...
//                 <p className="text-sm text-slate-400 mt-1 max-w-xs text-center">Connect your first social plateform to start scheduling and automating your content</p>
//             </div>
//         )
//     }
//   return (
...
//         const meta = PLATFORMS.find((p)=> p.id === account.palteform);
//         if(!meta) return null;
...
//                     <div className="text-sm text-slate-900 mt-0.5">{meta.name}</div>
//                 </div>
...
//                         <span className="text-xs text-amber-600">Disconneted</span>
//                         </>
//                     )}
//                 </div>

//                 <button onClick={()=>handleDisconnect(account._id)} title="Disconnet Account" className="ml-2 p-1.5 rounded-lg text-slate-300 group-hover:text-red-500 transition-all">
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                onClick={() => handleDisconnectInternal(account._id)}
