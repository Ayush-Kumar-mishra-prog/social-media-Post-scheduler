...
interface PlateformPickerModelProps {
  connectedIds: string[];
  connecting: string | null;
  onClose: () => void;
  onConnect: (platformId: string) => void;
}
const PlateFormPickerModel = ({
  connectedIds,
  connecting,
  onClose,
  onConnect,
}: PlateformPickerModelProps) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-100">
        <div className="flex items-center justify-between px-6 py-4 shadow">
          ...
          <button
            onClick={onClose}
            className="p-2 rouded-full hover:bg-slate-100 text-slate-500 transition-colors"
          >
          ...
          {PLATFORMS.map((p) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${isConnected ? "border-red-200 bg-red-50 cursor-default" : "border-slate-200 bg-salte-50 hover:border-slate-300 hover:bg-slate-100 cursor-pointer"} ${isConnecting && "opacity-60"}`}
              >
                <div className="p-2">
                  <p.icon
...
export default PlateFormPickerModel;
